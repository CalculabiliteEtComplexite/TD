...
\begin{exercice}[Modèles de machines de Turing]\label{exo:turingmachines/expressiveness/models}
  ...
  \item Construire une machine du modèle étendu qui décide le langage $\{a^{n^2} \mid n\in \mathbb{N} \}$.
    ...
    \emph{Remarque :} on rappelle que $\sum_{i=0}^n (2i+1) = n^2$.
  \end{question}
  \begin{correction}
    \begin{tikzpicture}[turingMachine, baseline=(0), x=40mm]
      ...
      \path (0) edge[loop below] node {\smGroup{\smTMtransR[\textsc{i}]{1}{1}\smTMtransR[\textsc{n}]{1}{1}}}                     (0);
      \path (1) edge[loop below] node {\smGroup{\smTMtransR[\textsc{i}]{1}{1}\smTMtransL[\textsc{n}]{1}{1}}}                     (1);
      \path (0) edge[bend left]  node {\smGroup{\smTMtransS[\textsc{i}]{1}{1}\smTMtransS[\textsc{n}]{\blank}{1}}}                (1);
      \path (1) edge             node {\smGroup{\smTMtransR[\textsc{i}]{1}{1}\smTMtransR[\textsc{n}]{\blank}{\blank}}}           (0);
      \path (0) edge             node {\smGroup{\smTMtransL[\textsc{i}]{\blank}{\blank}\smTMtransL[\textsc{n}]{\blank}{\blank}}} (2);
    \end{tikzpicture}
  \end{correction}
